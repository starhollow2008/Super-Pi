...
- Long-term memory lives in a SQLite store (`~/.pi/agent/memory/memory.db`). Tools: `memory_write` (store), `memory_search` (recall), `memory_list` (list). Toggle: `/memory on|off` (setting `memory.enabled` in settings.json).
...
- What qualifies: durable preferences, facts, project constraints, workflows. What does NOT: transient session state, task lists, anything the user hasn't seen the text of, API-keys and other sensitive data.
